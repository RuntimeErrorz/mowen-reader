...
import { Modal, Pressable, ScrollView, StatusBar as NativeStatusBar, Text, TextInput, View } from 'react-native';
  ...
  return (
    <Modal visible={visible} animationType="slide" statusBarTranslucent onRequestClose={onClose}>
      <SafeAreaView edges={['top', 'left', 'right', 'bottom']} style={[styles.settingsPage, { backgroundColor: palette.bg }]}>
        <NativeStatusBar translucent backgroundColor={palette.bg} barStyle={palette.bg === '#142428' ? 'light-content' : 'dark-content'} />
        ...
        <ScrollView contentContainerStyle={styles.settingsScroll} keyboardShouldPersistTaps="handled">
          ...
          <Field palette={palette} label="自定义 JSON 请求参数 · 可选" value={draft.customRequestParams} onChangeText={(customRequestParams) => setDraft({ ...draft, customRequestParams })} placeholder={'{"reasoning_effort":"medium"}'} autoCapitalize="none" autoCorrect={false} multiline />
        </ScrollView>
      </SafeAreaView>
    </Modal>
  );
...
function Field(props: React.ComponentProps<typeof TextInput> & { label: string; palette: ReaderPalette }) {
  ...
  return <View style={styles.field}><Text style={[styles.fieldLabel, { color: palette.text }]}>{label}</Text><TextInput {...inputProps} placeholderTextColor={palette.muted} style={[styles.fieldInput, inputProps.multiline && styles.fieldInputMultiline, { backgroundColor: palette.control, borderColor: palette.line, color: palette.text }]} /></View>;
}
