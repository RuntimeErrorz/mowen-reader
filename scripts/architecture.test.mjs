...
test('AI thinking stays separate from answers and image swipes stay page gestures', async () => {
  ...
  const aiPanel = await read('src/components/reader/AIPanel.tsx');
  const conversationViewer = await read('src/components/reader/ConversationViewerModal.tsx');
  const answerMarkdown = await read('src/components/reader/AIMessageMarkdown.tsx');
  ...
  assert.doesNotMatch(aiPanel, /AIStreamingAnswer/);
  assert.doesNotMatch(conversationViewer, /AIStreamingAnswer/);
  assert.match(answerMarkdown, /<Markdown markdownit=\{aiMarkdown\}/);
  assert.doesNotMatch(answerMarkdown, /streaming/);
  assert.match(aiPanel, /<View style=\{styles\.historyAnswerBlock\}>/);
  assert.match(conversationViewer, /<View style=\{styles\.historyAnswerBlock\}>/);
  const uiStyles = await read('src/ui/styles.ts');
  assert.doesNotMatch(uiStyles, /historyAnswerBlock: \{[^\n]*(?:backgroundColor|borderWidth)/);
  assert.match(uiStyles, /latestBottomSpacer: \{ height: 16 \}/);
  assert.match(uiStyles, /answerMessageTime: \{ marginTop: -6 \}/);
  assert.match(uiStyles, /historyLastAnswerBlock: \{ paddingBottom: 0, marginBottom: 0 \}/);
  assert.match(uiStyles, /historyComposer: \{[^\n]*paddingTop: 8/);
  assert.match(uiStyles, /aiComposer: \{[^\n]*paddingTop: 8/);
  assert.match(aiPanel, /styles\.answerMessageTime/);
  assert.match(conversationViewer, /styles\.answerMessageTime/);
  assert.match(aiPanel, /styles\.historyLastAnswerBlock/);
  assert.match(conversationViewer, /styles\.historyLastAnswerBlock/);
  assert.doesNotMatch(uiStyles, /latestMessageTime/);
  assert.match(aiPanel, /<View style=\{styles\.latestBottomSpacer\} \/>/);
  assert.match(conversationViewer, /<View style=\{styles\.latestBottomSpacer\} \/>/);
  assert.doesNotMatch(thinkingTrace, /ActivityIndicator/);
  assert.doesNotMatch(aiPanel, /ActivityIndicator|正在生成回复/);
  assert.doesNotMatch(conversationViewer, /ActivityIndicator|正在生成回复/);
  assert.doesNotMatch(aiPanel, /answerTimestamp|setAnswerTimestamp/);
  assert.doesNotMatch(conversationViewer, /answerTimestamp|setAnswerTimestamp/);
  assert.match(aiPanel, /enableThinking: thinkingEnabled/);
  assert.match(autoScroll, /followLatestRef/);
  assert.match(autoScroll, /userScrolledAwayRef/);
  assert.match(aiPanel, /onFocus=\{handleInputFocus\}/);
  assert.match(conversationViewer, /onFocus=\{handleInputFocus\}/);
  assert.match(autoScroll, /scrollToEnd/);
  assert.match(autoScroll, /beginFollowing[\s\S]*scrollToLatest\(\);/);
  assert.doesNotMatch(autoScroll, /beginFollowing[\s\S]*scrollToLatest\(true\)/);
  assert.match(aiPanel, /onLayout=\{handleLayout\}/);
  assert.match(conversationViewer, /onLayout=\{handleLayout\}/);
  ...
  assert.match(aiPanel, /onScrollBeginDrag=\{handleScrollBeginDrag\}/);
  assert.match(conversationViewer, /onScrollBeginDrag=\{handleScrollBeginDrag\}/);
  assert.match(autoScroll, /userDraggingRef/);
